...
decision is available, load the same token and resume with resolutions for one
or more pending interruptions. This example resolves all of them:
...
contains the full batch, and the host can resolve any non-empty subset of it.
Unresolved calls keep their IDs and payloads and remain pending without being
invoked again. Resolved calls can advance independently and reach new host
calls or interruptions. If the run remains interrupted, save the returned
continuation and pending interruptions for the next resume. Apply subsequent
resolutions to that updated continuation so completed work is preserved.
